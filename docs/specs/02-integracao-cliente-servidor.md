...
| 2.6.4 ✅ | **ENTREGUE (S18)**: broadcast no `GameServer.tick()` — payload base drenado 1×/tick e serializado (envelope **v3**) por cliente; **`viewFor(player)` como identidade** (D-2.6-D) com o contrato da filtragem documentado; backpressure por drop de tick (`bufferedAmount > 256 KB`); métricas (`getNetworkStats`); eventos sem duplicação/acúmulo | **2 clientes reais, 60 snapshots, 0 divergências** + **71 asserts** (broadcast: 6 novos, contrato congelado ANTES) + série medida **1.388 B/tick · 26,7 KB/s · 19,4 Hz** + gráfico canônico registrado como evidência |

...
> **Próximo passo obrigatório**: fechar o **grill-me das decisões 2.6.x** (`docs/decisions/2026-09-10_fase-2.6.x-abertas.md`) e iniciar a **sub-fase 2.6.2** (paridade de modelo: custos, POP, tempos no servidor).
...
**Sessão 15 (Workspace `shared/` — Fase 2.6.1, 10/09 21:05 BRT)**: extração entregue — `@project-exodus/shared` (protocol/units/economy/world) consumido por cliente e servidor via `dist`, com o `composite` do `tsconfig.base.json` finalmente em uso (MED-07 fechado). Tipos canônicos únicos (`UnitType` 5, `BuildingType` 3), stats/custos/tempos, layout dos 8 veios e spawns numa fonte só; re-exports mantêm compatibilidade dos consumidores; **zero mudança de comportamento** (valores decididos mas ainda não aplicados marcados `LEGACY (2.6.1)` com a sub-fase que os aplica). Divergências residuais descobertas na extração e agendadas para a 2.6.3: `DROPOFF_RANGE` (cliente 10 × servidor 14) e clamp de mundo (±88 × ±90) — decisões propostas no novo grill-me. Gates: `tsc` 0 em 4 workspaces, 43 asserts, build 689 kB, `dist-proof` 11/8/0, `collision-e2e` 8,701/3,101, `gather-e2e`, `test-buttons` 10/10. Ver `docs/journal/2026-09-10_21-05_sessao-15-workspace-shared.md`.
